...
import InspectionTab from './evaluate_tab_with_inspection';
...
function EvaluateModule() {
  ...
  const [activeTab, setActiveTab] = React.useState<'browse' | 'compare' | 'inspection'>('browse');
  const subTabs = [
    { key: 'browse' as const, label: 'Browse' },
    { key: 'compare' as const, label: 'Compare' },
    { key: 'inspection' as const, label: 'Inspection' },
  ];
  const currentIndex = subTabs.findIndex((t) => t.key === activeTab);
  ...
  return (
    <div className="p-4 max-w-6xl mx-auto" {...handlers}>
      {!active ? (
        <div>
          {/* Clickable sub-tabs + arrows (Browse / Compare / Inspection) */}
          <div className="flex items-center justify-center gap-3 mb-2">
            ...
            <button
              ...
              onClick={() => {
                if (currentIndex > 0) {
                  setActiveTab(subTabs[currentIndex - 1].key);
                }
              }}
            >
              ←
            ...
            {/* Center tab buttons */}
            <div className="flex items-center gap-4 rounded-full bg-white px-3 py-1 border"
                 style={{ borderColor: COLORS.lightGray }}>
              {subTabs.map((t) => {
            ...
            <button
              ...
              style={{
                borderColor: COLORS.lightGray,
                opacity: currentIndex === subTabs.length - 1 ? 0.3 : 1,
              }}
              aria-label="Next tab"
              onClick={() => {
            ...
            {/* Small hint text */}
            <div className="ml-3 text-[11px]" style={{ color: COLORS.gray }}>
              {activeTab === 'browse'
                ? 'Swipe left to Compare'
                : activeTab === 'compare'
                ? 'Swipe right to Browse'
                : 'Click arrows to switch tabs'}
            </div>
          </div>
